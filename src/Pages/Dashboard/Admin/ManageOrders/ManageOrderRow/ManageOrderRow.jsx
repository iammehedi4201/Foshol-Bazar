import React from 'react';
import { Link } from 'react-router-dom';
import useAxiosSecure from '../../../../../Hooks/useAxiosSecure';
import Swal from 'sweetalert2';

const ManageOrderRow = ({ order, orderRefetch }) => {
    const { _id, customerName, customerEmail, phoneNumber, date, shippingAddress, status, orderItems } = order;
    const [axiosSecure] = useAxiosSecure()


    const handleUpdateStatus = async (id) => {
        Swal.fire({
            title: 'Are you sure?',
            text: "You Want to Change Status",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, Change it!'
        }).then(async (result) => {
            if (result.isConfirmed) {
                const response = axiosSecure.patch(`/orders/${id}`)
                    .then(response => {
                        if (response.data.modifiedCount > 0) {
                            orderRefetch();
                            Swal.fire(
                                'Updated!',
                                'Status has been updted.',
                                'success'
                            )
                        }
                    })
            }
        })
    }

    return (
        <tr className="bg-black border-4 border-slate-800 hover:bg-slate-800 ">
            <td className="px-6 py-4 font-bold text-gray-100 shadow-xl shadow-cyan-600 text-center ">
                {customerEmail}
            </td>
            <td className="px-6 py-4 font-bold text-gray-100 shadow-xl shadow-cyan-600 text-center ">
                {phoneNumber}
            </td>
            <td className="px-6 py-4 font-bold text-gray-100 shadow-xl shadow-cyan-600 text-center ">
                {shippingAddress?.address}
            </td>
            <td className="px-6 py-4 font-bold  text-gray-100 shadow-xl shadow-cyan-600 text-center ">
                {date}
            </td>
            <td className="px-6 py-4  font-bold  text-gray-100 shadow-xl shadow-cyan-600 text-center">
                {status}
            </td>
            <td className="px-6 py-4 flex flex-col sm:flex-row flex-wrap  justify-center sm:justify-center items-center gap-2">
                <Link className="btn font-medium text-white bg-black hover:bg-slate-600 mx-5">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                        <path d="M12 15a3 3 0 100-6 3 3 0 000 6z" />
                        <path fill-rule="evenodd" d="M1.323 11.447C2.811 6.976 7.028 3.75 12.001 3.75c4.97 0 9.185 3.223 10.675 7.69.12.362.12.752 0 1.113-1.487 4.471-5.705 7.697-10.677 7.697-4.97 0-9.186-3.223-10.675-7.69a1.762 1.762 0 010-1.113zM17.25 12a5.25 5.25 0 11-10.5 0 5.25 5.25 0 0110.5 0z" clip-rule="evenodd" />
                    </svg>

                </Link>
                <Link onClick={() => handleUpdateStatus(_id)} className="btn font-medium text-white bg-[#80E2B7] hover:bg-[#4ca47e]">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                        <path d="M20 6L9 17L4 12" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </Link >
            </td>
        </tr>
    );
};

export default ManageOrderRow;